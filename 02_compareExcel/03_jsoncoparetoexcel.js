...
rl.question("Please enter the folder name: ", (folderName) => {
  const folderPath = `03_JsonExcelformat`;
  const folderPath2 = `03_JsonExcelformatTruth`;
  ...
  folderFiles.forEach((folder) => {
    ...
    const targetFolderPath = path.join("04_ExcelOutput/", folderName);
    fs.mkdirSync(targetFolderPath, { recursive: true });
    const existingFilePath = `04_ExcelOutput/${folderName}/${folder
      .split("/")
